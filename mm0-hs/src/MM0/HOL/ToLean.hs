...
wrapAt :: Bool -> Int -> String -> String
wrapAt cmt margin = \s -> fromMaybe s (go id 0 s) where
  newline f s = if cmt then go (f . ("\n-- " ++)) 3 s else go (f . ('\n' :)) 0 s
  go f _ ('\n' : s) = newline f s
  go _ col (' ' : _) | col > margin = Nothing
  go f col (' ' : s) = go (f . (' ' :)) (col+1) s <|> newline f s
  go _ col (_ : _) | col >= margin = Nothing
  go f col (c : s) = go (f . (c :)) (col+1) s
  go f _ [] = Just (f [])

leanComment :: Bool -> Comment -> LeanM ()
leanComment cmt c =
  forM_ c $ \s -> emit $ wrapAt cmt 100 $ "\n/-- " ++ T.unpack s ++ " -/"

writeLean :: Axioms -> String -> String -> Int -> [WithComment HDecl] -> IO ()
writeLean ax f ns chunkSize ds =
  evalStateT (do
      open (axFile ax)
      forM_ ds $ \d -> do
        leanDecl ax d
        increment
      close) $
    LeanState 1 f ns undefined 0 chunkSize M.empty
...
open i = do
  ...
  emit $ "namespace " ++ lNamespace g ++ "\n"

close :: LeanM ()
close = do
  g <- get
  emit $ "end " ++ lNamespace g ++ "\n"
...
printProof _ _ (HHyp v []) = mangle v
printProof p _ (HHyp v xs) = showParen p $
  mangle v . flip (foldr (\x -> (' ' :) . mangle x)) xs
printProof _ _ (HThm t [] [] []) = mangle t
...
printProof _ _ HSorry = ("sorry" ++)
printProof _ _ HSave {} = error "proof should be unsaved"
...
unsaveProof ctx (HThm t es hs xs) = do
  ...
  (ss2, hs') <- mapAndUnzipM (unsaveProofLam ctx) hs
  return (s1 <> fold ss2 <> S.fromList xs, HThm t es hs' xs)
unsaveProof ctx (HSave v p xs) = do
  (s, HProofLam bis2 p') <- unsaveProofLam ctx p
  let go _ [] bis1 bis' xs' = (bis1, bis', xs')
      go s' (vt@(x, _) : ctx') bis1 bis' xs' =
        if S.member x s && S.notMember x s' then
          go (S.insert x s') ctx' (x : bis1) (vt : bis') (x : xs')
        else go s' ctx' bis1 bis' xs'
  let (bis1, bis', xs') = go S.empty ctx [] bis2 xs
...
unsaveProof _ HSorry = return (S.empty, HSorry)
...
leanDecl :: Axioms -> WithComment HDecl -> LeanM ()
leanDecl ax (WC c (HDSort s)) = do
  leanComment (noAx ax) c
  let s' = mangle s
  let emit' = emit . if noAx ax then ("-- " ++) else id
...
leanDecl ax (WC c (HDTerm x ty)) = do
  leanComment (noAx ax) c
  let emit' = emit . if noAx ax then ("-- " ++) else id
  emit' $ "constant " ++ mangle x " : " ++ printHType ty "\n"
leanDecl ax (WC c (HDDef x ss xs r t)) = when (ax /= Only) $ do
  leanComment False c
  let bis = printGroupedBinders False (ss ++ (mapSnd (SType []) <$> xs))
  emit $ ("def " ++) $ mangle x $ bis (" : " ++ T.unpack r ++ " :=\n" ++ printTerm False t "\n")
leanDecl ax (WC c (HDThm x (TType vs gs ret) Nothing)) = do
  leanComment (noAx ax) c
  let emit' = emit . if noAx ax then ("-- " ++) else id
  emit' $ ("axiom " ++) $ mangle x $ printGroupedBinders True vs $ " : " ++
    foldr (\g r -> printGType True g (" \x2192 " ++ r)) (printGType False ret "\n") gs
leanDecl ax (WC c (HDThm x (TType vs gs (GType xs ret)) (Just (hs, pr)))) = when (ax /= Only) $ do
  leanComment False c
