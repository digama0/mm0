...
impl<'a> TermRef<'a> {
    ...
    pub fn sort(&self) -> SortId {
        SortId(self.sort & 0x7F)
    }
    /// The list of arguments of this term/def, not including the return).
    #[inline]
    #[must_use]
    pub fn args_no_ret(&self) -> &[Arg] {
        self.args.split_last().expect("nonempty").1
    }
    ...
    #[must_use]
    pub fn ret(&self) -> Arg {
        *self.args.last().expect("nonempty")
    }
...
fn try_next_decl(mmb: &[u8], pos: usize) -> Result<Option<(StmtCmd, ProofIter<'_>, usize)>, ParseError> {
...
impl<'a> IndexEntryRef<'a> {
    ...
    pub fn left(&self) -> Option<Self> {
        index_ref(self.buf, self.p_left)
    }
    /// The right child of this entry.
    #[must_use]
    pub fn right(&self) -> Option<Self> {
        index_ref(self.buf, self.p_right)
    }
