...
impl<'a> Parser<'a> {
    ...
    #[must_use]
    pub fn cur_opt(&self) -> Option<u8> {
        self.source.get(self.idx).cloned()
    }
        ...
        while self.idx < self.source.len() {
            ...
            match c {
                ...
                b'\r' => {
                    ...
                    self.errors.push(ParseError::new(
                        start..self.idx,
                        "carriage return (windows style newlines) are not permitted in MM0 files"
                            .into(),
                    ))
                }
                ...
                b'\\' => s.push(match (self.cur_opt(), self.idx += 1).0 {
                    ...
                    Some(b'x') | Some(b'X') if self.idx + 2 <= self.source.len() => {
                        ...
                        if let (Some(h1), Some(h2)) =
                            ((c1 as char).to_digit(16), (c2 as char).to_digit(16))
                        {
