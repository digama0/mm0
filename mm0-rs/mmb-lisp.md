...
| `4`    | `Dyn`            | `len: uleb`, `[u8; len]` | **stubbed** (below)

`Dyn` wraps a `Box<dyn LispProc>`, an external object — today the MMC compiler — whose contents only it can write. Serializing it needs a serialize/deserialize hook on the [`LispProc`] trait; the format reserves `kind = 4` with a length-prefixed blob for that hook, but the hook is not yet defined, so the exporter stubs it.

[`LispProc`]: https://github.com/digama0/mm0/blob/master/mm0-rs/src/elab/lisp.rs
